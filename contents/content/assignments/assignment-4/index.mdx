---
title: Assignment 4 - Routing and API calls
---

_First, complete [assignment 3](/content/assignments/assignment-3) if you haven't already.
...
In workshop 4, we covered client-side routing and added new routes to an ecommerce app
...
In assignment 4, you’ll add routing and API calls to your web app.

## Setup

1. You’ll begin with your web app from assignment 3.
   We will build off of that repository this week.

2. Navigate to your local project repository from assignment 3.
   Make sure you are on the `hw-3` branch
   (or whatever branch contains all your changes from assignment 3). 
   - Run `git branch` to check what branch you are on. 
   - Run `git checkout branch-name` to switch to a different branch.

3. Run `git checkout -b hw-4` to create a new branch called “hw-4” that
   branches off your hw-3 branch.
   **All of the work on your project MUST be done on this branch.
   You should NOT modify the main branch, or the hw-3 branch.**

4. Run `git push -u origin hw-4` to publish this new branch on github.

5. Dedicate one terminal window to running your code with npm.

   - Run `npm run dev` in this terminal to run your app and view your changes in the browser.
...
7. Now you are ready! Go forth and code!
   - As you progress on this assignment, continue grouping your changes into commits and
     pushing them to the hw-4 branch on github.
     All of your commits MUST be on the hw-4 branch, not the main branch or the hw-3 branch.
...
- A machine in Ohio is set up with the code to execute these functions.
  ...
  - The domain/hostname of the server is: `https://disc-assignment-5-users-api-iyct.onrender.com` (yes it says assignment 5 just ignore that 😇)
...
- For example, say I wanted to delete the user named “Bob”.
  - First, I make a `GET` request to “https://disc-assignment-5-users-api-iyct.onrender.com/api/users”
  ...
  - Finally, I make a `DELETE` request to
    “https://disc-assignment-5-users-api-iyct.onrender.com/api/users/28” to delete the user with id 28.
...
- **Important note**: the server will go to sleep if it hasn’t received any requests in a long time.
  ...
  - To wake the server, you can make a `GET` request to it by
    opening this URL in your browser: https://disc-assignment-5-users-api-iyct.onrender.com/api/users.
...
2. On github, make a pull request from your `hw-4` branch.
   **Do NOT merge the pull request**.
3. Once you've created the pull request, copy its URL from the browser.
4. Fill out [this google form](https://forms.gle/6N8NU393cpEokVPR7)
